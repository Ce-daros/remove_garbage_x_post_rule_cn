...
// @version      6.46.1
// @description  防误伤强化版：垃圾推文自动隐藏；垃圾号排队拉黑；远程规则自动更新；自定义关键词管理。
// @author       summeriscoming/小雨
// @license MIT
...
(function () {
  ...
  const REMOTE_RULES_URL = 'https://raw.githubusercontent.com/Ce-daros/remove_garbage_x_post_rule_cn/refs/heads/main/rules.json';
  ...
  function runHidePrewarm() {
    ...
    for (const art of document.querySelectorAll('article[data-testid="tweet"]')) {
      ...
      if (art.dataset.xfsPrewarmChecked === '1') continue;
      const rect = art.getBoundingClientRect();
      if (rect.top < cutoffTop) continue;
      art.dataset.xfsPrewarmChecked = '1';
  ...
  function scheduleHidePrewarm(delay = 900, opts = {}) {
    ...
    hidePrewarmTimer = setTimeout(() => {
      ...
      if (window.requestIdleCallback) hidePrewarmIdleHandle = window.requestIdleCallback(run, { timeout: 1200 });
      else hidePrewarmIdleHandle = setTimeout(run, 80);
    }, delay);
  }
  ...
  */
  ...
  function startUI() {
    ...
    function scheduleVisibleRuleRepair(delay = 650) {
      clearTimeout(visibleRuleRepairTimer);
